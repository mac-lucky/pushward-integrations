...
// Handler processes Proxmox VE webhooks for multiple tenants.
type Handler struct {
	store   state.Store
	clients *client.Pool
...
// NewHandler creates a new Proxmox VE webhook handler.
...
// ServeHTTP handles incoming Proxmox VE webhook requests.
...
// handleVzdump processes backup lifecycle events.
...
// replicationJobRe extracts the job identifier (e.g. "101/0") from replication messages.
var replicationJobRe = regexp.MustCompile(`(?:job|Job)\s+([\d/]+)`)

// handleReplication processes replication lifecycle events.
func (h *Handler) handleReplication(ctx context.Context, userKey string, p *webhookPayload) {
	// Extract a stable job identifier from the message to ensure slug consistency
	// across start and finish events (titles differ between phases).
	jobID := "unknown"
...
// handleFencing processes fencing point-in-time events.
...
// handleUpdates processes package-updates informational events.
