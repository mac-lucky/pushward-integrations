...
func (h *Handler) handleWebhook(ctx context.Context, input *struct {
	...
	if ev == nil {
		...
		if err := selftest.SendTest(ctx, h.clients.Get(userKey), log, "bazarr"); err != nil {
...
func (h *Handler) handleSubtitle(ctx context.Context, userKey string, log *slog.Logger, ev *subtitleEvent) error {
	...
	req := pushward.SendNotificationRequest{
		Title:      action + text.SepDot + ev.language,
		Subtitle:   ev.media,
		Body:       ev.media + text.SepDot + action + text.SepDot + ev.language + text.SepDot + ev.score + "%",
