...
// running (e.g. a hung self-hosted runner, which is the normal deployment for
// Forgejo Actions) that would otherwise block new-run detection for the repo
// indefinitely. Set well above any realistic job timeout so legitimate long runs
...
const activityIcon = "arrow.triangle.branch"

func New(cfg *config.Config, fj *fjclient.Client, pw *pushward.Client) *Poller {
	return &Poller{
		cfg:     cfg,
		fj:      fj,
		pw:      pw,
...
func (p *Poller) Run(ctx context.Context) error {
	defer func() {
		// Collect the pending end-timer groups under the lock, then Close + Wait
		// OUTSIDE it: an in-flight phase callback re-acquires p.mu, so waiting
		// under the lock would deadlock. Close (not Stop) prevents a phase-1
		// callback from re-arming phase 2 after we've stopped it.
		p.mu.Lock()
...
func (p *Poller) refreshRepos(ctx context.Context) error {
	...
	discovered, err := p.fj.ListRepos(ctx, p.cfg.Forgejo.Owner)
	if err != nil {
		return err
	}

	// Merge: discovered repos + any explicitly configured repos
	seen := make(map[string]bool, len(discovered)+len(p.cfg.Forgejo.Repos))
	var merged []string
	...
	for _, r := range p.cfg.Forgejo.Repos {
		if !seen[r] {
			seen[r] = true
			merged = append(merged, r)
		}
...
func (p *Poller) pollIdle(ctx context.Context) error {
	for _, repo := range p.repos {
		...
		runs, err := p.fj.GetActiveRuns(ctx, repo)
		if err != nil {
		...
		slug := text.SlugHash(slugPrefix, repo, 4)

		slog.Info("workflow found",
			"repo", repo, "run_id", run.ID, "run", run.IndexInRepo,
			"workflow", run.Name, "branch", run.HeadBranch, "event", run.Event, "slug", slug)
		...
		if err := p.pw.CreateActivity(ctx, slug, fmt.Sprintf("Forgejo: %s", repoShort), p.cfg.PushWard.Priority, endedTTL, staleTTL); err != nil {
		...
		// Determine the initial step shape. Forgejo creates jobs lazily within a
		...
		if jobs, err := p.fj.GetLiveJobs(ctx, repo, run.ID); err != nil {
		...
		repoURL := p.tracked[repo].RepoURL
		p.mu.Unlock()
		...
		if err := p.pw.UpdateActivity(ctx, slug, pushward.UpdateRequest{
			...
			Content: pushward.Content{
				...
				// The API's own html_url, never one built locally: Forgejo derives it
				// from the run's index_in_repo, not the id this bridge fetches by.
				URL:          run.HTMLURL,
				SecondaryURL: repoURL,
			},
		}); err != nil {
...
func (p *Poller) repoURL(repo string, run fjclient.Run) string {
	if run.RepoHTMLURL != "" {
		return run.RepoHTMLURL
	}
	return p.fj.WebBase() + "/" + repo
}
...
func (p *Poller) shape(jobs []fjclient.Job) ci.StepInfo {
...
// branch ran most recently, whose job shape may differ. An empty workflow file
// name likewise can't target a workflow, so both short-circuit to the live scan.
//
...
func (p *Poller) baselineShape(ctx context.Context, repo, workflowID, branch string) (ci.StepInfo, bool) {
	if workflowID == "" || branch == "" {
		return ci.StepInfo{}, false
	}
	prev, err := p.fj.GetLatestFinishedRun(ctx, repo, workflowID, branch)
	if err != nil {
		slog.Warn("failed to look up prior run for step seed",
			"repo", repo, "workflow", workflowID, "branch", branch, "error", err)
		return ci.StepInfo{}, false
	}
	if prev == nil {
		return ci.StepInfo{}, false
	}
	// Ask for the timing join only when something consumes it. Forgejo's job
	// objects carry no timestamps, so the durations cost an extra paginated call
	// that is pure waste when both pill sizing and the ETA are switched off.
	var jobs []fjclient.Job
	if p.cfg.Render.StepWeights || p.cfg.Render.LiveProgress {
		jobs, err = p.fj.GetFinishedJobs(ctx, repo, prev.ID, prev.IndexInRepo)
	} else {
		jobs, err = p.fj.GetJobs(ctx, repo, prev.ID)
	}
	if err != nil {
...
func (p *Poller) pollActive(ctx context.Context) error {
	...
	for _, repo := range repos {
		...
		// runs that disappeared, NOT for runs stuck running.
		...
		// Eviction guard 2: absolute age. A run wedged running (hung self-hosted
		// runner) keeps returning jobs, so LastUpdate never expires and the bridge
		// would track it - and block new-run detection for the repo - forever.
		// Reclaim it past a generous lifetime ceiling.
		...
		jobs, err := p.fj.GetLiveJobs(ctx, tRepo, tRunID)
		if err != nil {
		...
		if tt, ok := p.tracked[repo]; ok {
			...
			// Clamp TotalSteps to never decrease: Forgejo lazily creates jobs
			// behind needs/if conditions, so new steps appear over time. We keep
			// the highest total to avoid confusing jumps.
			if info.TotalSteps > tt.maxTotalSteps {
				...
				// Fewer groups than the seeded maximum: a wave Forgejo has not
		...
		// each weight tracks its label regardless of the order Forgejo reveals the
		...
		if info.AllCompleted {
			// All *visible* jobs are done, but Forgejo creates jobs lazily
			// (if-gated jobs, dependent matrices). Confirm the run itself finished
			// before ending - otherwise a poll landing between job waves would
			// prematurely dismiss the Live Activity.
			run, err := p.fj.GetRun(ctx, tRepo, tRunID)
			if err != nil {
			...
			if !run.Terminal() {
				...
				slog.Debug("visible jobs complete but run still in progress, deferring end",
					"repo", repo, "run_id", tRunID, "status", run.RawStatus)
				continue
			}
			// The run's own status is authoritative for the final outcome. Forgejo
			// has no separate conclusion field, so this is the one value to read.
			state, color := runOutcome(*run, info.AnyFailed)
			slog.Info("workflow completed", "run_id", tRunID, "slug", tSlug, "status", run.RawStatus, "state", state)
		...
		// Send only when a scalar changed, Forgejo revealed new jobs (shape grew),
		...
		// step_rows/step_labels are re-sent only when Forgejo lazily revealed new
...
// runOutcome maps a terminal run to its final card state and accent color.
//
...
func runOutcome(run fjclient.Run, anyFailed bool) (state, color string) {
...
// ensures the Dynamic Island shows the final state before dismissal.
