...
import (
	...
	"github.com/mac-lucky/pushward-integrations/starr/internal/handler"
)
...
func main() {
	...
	mux := server.NewMux()
	mux.HandleFunc("/radarr/webhook", h.HandleRadarrWebhook)
	mux.HandleFunc("/sonarr/webhook", h.HandleSonarrWebhook)

	ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
	defer cancel()

	slog.Info("starting pushward-starr",
		"address", cfg.Server.Address,
		"priority", cfg.PushWard.Priority,
		"cleanup_delay", cfg.PushWard.CleanupDelay,
		"stale_timeout", cfg.PushWard.StaleTimeout,
	)
