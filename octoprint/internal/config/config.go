...
type Config struct {
	...
	OctoPrint OctoPrintConfig             `yaml:"octoprint"`
...
func Load(path string) (*Config, error) {
	...
	if v := os.Getenv("PUSHWARD_OCTOPRINT_URL"); v != "" {
		cfg.OctoPrint.URL = v
	}
	if v := os.Getenv("PUSHWARD_OCTOPRINT_API_KEY"); v != "" {
		cfg.OctoPrint.APIKey = v
	}
	if v := os.Getenv("PUSHWARD_OCTOPRINT_WEBHOOK_SECRET"); v != "" {
		cfg.OctoPrint.WebhookSecret = v
	}
	...
	if cfg.OctoPrint.URL == "" {
		return nil, fmt.Errorf("octoprint.url is required (set PUSHWARD_OCTOPRINT_URL)")
	}
	if cfg.OctoPrint.APIKey == "" {
		return nil, fmt.Errorf("octoprint.api_key is required (set PUSHWARD_OCTOPRINT_API_KEY)")
	}
